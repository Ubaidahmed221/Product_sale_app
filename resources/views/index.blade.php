...
<!-- Offer End -->
  ...
  <div class="container-fluid pt-5">
    ...
    <div class="row px-xl-5 pb-3">
        ...
        @foreach (getTrandyProducts() as $trandy)
            
        <div class="col-lg-3 col-md-6 col-sm-12 pb-1">
            <div class="card product-item border-0 mb-4">
                
                <div class="card-header product-img position-relative overflow-hidden bg-transparent border p-0">
                    <img class="img-fluid w-100" src="{{ $trandy->firstImage->path }}"alt="">
                </div>
                <div class="card-body border-left border-right text-center p-0 pt-4 pb-3">
                    <h6 class="text-truncate mb-3">{{ $trandy->title }}</h6>
                    <div class="d-flex justify-content-center">
                        {{-- <h6>$123.00</h6><h6 class="text-muted ml-2"><del>$123.00</del></h6> --}}
                        @if (getUserCurrency())
                            
                        <h6>Rs {{ number_format($trandy->pkr_price,2) }}</h6>
                        <h6 class="text-muted ml-2"><del>Rs {{ number_format($trandy->pkr_price,2) }}</del></h6>

                        @else
                        
                        <h6>${{ number_format($trandy->usd_price,2) }}</h6>
                        <h6 class="text-muted ml-2"><del>${{ number_format($trandy->usd_price,2) }}</del></h6>
                        @endif
                    </div>
                </div>
                <div class="card-footer d-flex justify-content-between bg-light border">
                    <a href="{{ route('product.detail',\Crypt::encrypt($trandy->id)) }}" class="btn btn-sm text-dark p-0"><i class="fas fa-eye text-primary mr-1"></i>View Detail</a>
                    <a class="btn btn-sm text-dark p-0 add-to-cart " data-product-id="{{$trandy->id}}" ><i class="fas fa-shopping-cart text-primary mr-1"></i>Add To Cart</a>
                </div>
            </div>
        </div>
        @endforeach
